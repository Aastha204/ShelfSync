...
const ReceiptView = () => {
  ...
  // Function to handle downloading the receipt as a PDF (for demonstration purposes)
    ...
    const receiptHTML = `
      <div style="font-family: 'Roboto', Arial, sans-serif; max-width: 520px; margin: 50px auto; padding: 25px 30px; border-radius: 16px; background-color: #f5f5dc; color: #3e2723; box-shadow: 0 8px 24px rgba(0, 0, 0, 0.2);">
        ...
        <div style="background: #faf3e0; border-radius: 12px; padding: 15px; margin-bottom: 20px; border: 1px solid #8b5e3c;">
          ...
          <p style="margin: 5px 0;"><strong>Borrower's Name:</strong> ${
            receipt.userName
          }</p>
          <p style="margin: 5px 0;"><strong>Receipt No:</strong> ${
            receipt.receiptNo
          }</p>
        </div>
        ...
        <div style="background: #fdf8f0; border-radius: 12px; padding: 15px; margin-bottom: 20px; border: 1px solid #8b5e3c; box-shadow: inset 0 2px 5px rgba(0, 0, 0, 0.1);">
          <p style="margin: 8px 0;">📚 <strong>Book Name:</strong> ${
            receipt.bookName
          }</p>
          <p style="margin: 8px 0;">✍️ <strong>Author:</strong> ${
            receipt.authorName || "Unknown Author"
          }</p>
          <p style="margin: 8px 0;">🗓️ <strong>Issue Date:</strong> ${new Date(
            receipt.issueDate
          ).toLocaleDateString()}</p>
          <p style="margin: 8px 0;">🗓️ <strong>Return Date:</strong> ${
            receipt.returnDate
              ? new Date(receipt.returnDate).toLocaleDateString()
              : "Not Returned"
          }</p>
          <p style="margin: 8px 0;">💰 <strong>Price:</strong> ₹${
            receipt.price ? parseFloat(receipt.price).toFixed(2) : "N/A"
          }</p>
        </div>
  ...
  return (
    <div className="min-h-screen flex flex-col justify-center items-center py-5 bg-custom">
      ...
      <div
        className="bg-white w-full max-w-4xl md:max-w-3xl sm:max-w-lg border-4 border-brown-600 rounded-lg shadow-lg p-3"
        id="receipt-content"
      >
        {/* Header */}
